...
var LOAD_TRIP_URL;
const USER_ENDPOINT = 'http://calm-hollows-72370.herokuapp.com/users';
const TRIP_ENDPOINT = 'http://calm-hollows-72370.herokuapp.com/trip';
...
function login() {
      ...
      $('#loginPage').on('click', '#login', function (event) {
        ...
        const LOGIN_ENDPOINT = 'http://calm-hollows-72370.herokuapp.com/login/' + userLogin + '/' + passLogin + '/';
        $.getJSON(LOGIN_ENDPOINT, function(response) {
          status = response;
          if (status == "Success") {
            alert('Welcome to Planit!');
            ACTIVE_USER = userLogin;
            LOAD_TRIP_URL = 'http://calm-hollows-72370.herokuapp.com/trip/' + ACTIVE_USER;
...
function buildList() {
  ...
  list.map(function (item) {
      ...
      if (item == 'Review Travel Requirements') {
        ...
        YELP_SEARCH_URL = 'http://calm-hollows-72370.herokuapp.com/yelp/' + fromCity + '/' + service + '/';
